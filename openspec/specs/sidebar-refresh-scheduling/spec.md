# sidebar-refresh-scheduling Specification

## Purpose

Sidebar quota-refresh policy: a fetch-free clock that renders once per
second, plus a shared per-provider scheduler giving OpenCode Go, DeepSeek,
Ollama, and Kimi one immediate first load and a non-overlapping 30-second
automatic cadence measured from each attempt's settlement. Explicit
triggers (login, delete, manual) share the per-provider single-flight
boundary so they join in-flight requests instead of duplicating them.
