# delete-failure-surfacing Specification

## Purpose

The sidebar's account-delete confirm flow parses the `/api/delete` response
and surfaces failures to the user: a failed or uncertain delete keeps the
confirm modal open with the error message and refreshes nothing, while a
confirmed success closes the modal and refreshes only the deleted
provider's cards from the confirm-time snapshot. In-flight responses respect
modal ownership so a stale response never operates on a newer confirmation.
