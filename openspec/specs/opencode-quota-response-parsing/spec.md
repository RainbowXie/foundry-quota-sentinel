# opencode-quota-response-parsing Specification

## Purpose

Structured, fail-closed parsing of the OpenCode Go quota response, a
seroval-like serialization rather than plain JSON. The parser accepts the
evidence-backed layout variations (reference-number drift, property order,
whitespace, unrelated extra properties, inline objects) while rejecting
missing, malformed, truncated, duplicated, negative, or unsupported data
without fabricating values or substituting windows. All fixtures and
diagnostics stay free of authentication and account material.
