...
- Focused: all 6 delete-flow scenarios GREEN via the stub-DOM harness
  (success / failure / network / malformed / stale-success / stale-failure).
- Full `internal/web` suite: 75 tests green. `go test ./...` green.
- `gofmt` clean; `go test -race -tags nogui ./...` green; `go vet` (default
  AND `-tags nogui`) clean; GUI/nogui `go build` clean;
  `git diff --check` clean; `openspec validate --all` valid (5 items).
